            ...
            if ($request->has('message_id') && $request->message_id) {
                ...
                $mediaFile = MediaFiles::firstOrNew([
                    ...
                    if ($request->hasFile('recipient_image')) {
                        ...
                        \Illuminate\Support\Facades\Storage::disk(config('filesystems.media_disk'))->put($path, (string) $encodedImage);
                ...
                if ($request->hasFile('background_music')) {
                    ...
                    if ($mediaFile->background_music && !str_starts_with($mediaFile->background_music, 'http')) {
                        try {
                            \Illuminate\Support\Facades\Storage::disk(config('filesystems.media_disk'))->delete($mediaFile->background_music);
                        } catch (\Exception $e) {}
                    }
                        $mediaFile->background_music = $file->store('guest_media/background-music', config('filesystems.media_disk'));
                ...
                if ($request->hasFile('recipient_image')) {
                    $image = $request->file('recipient_image');
                    $mediaFile->recipient_image = $image->store('guest_media/recipient-images', config('filesystems.media_disk'));
                }
