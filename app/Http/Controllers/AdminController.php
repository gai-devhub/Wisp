...
class AdminController extends Controller
{
    ...
    public function settingsMessagePage(Request $request): View
    {
        ...
        foreach ($mediaFiles as $mf) {
            if ($mf->recipient_image) {
                try { $imagesSize += Storage::disk(config('filesystems.media_disk'))->size($mf->recipient_image); } catch (\Throwable $e) {}
            }
            if ($mf->background_music && !str_starts_with($mf->background_music, 'http')) {
                try { $audioSize += Storage::disk(config('filesystems.media_disk'))->size($mf->background_music); } catch (\Throwable $e) {}
            }
        }
        // Profile pictures
    ...
    public function messageDetails(Request $request, $id)
    {
        ...
        foreach ($message->mediaFiles as $media) {
            ...
            if ($media->file_type === 'image' && $media->file_path) {
                try { $size = Storage::disk(config('filesystems.media_disk'))->size($media->file_path); } catch (\Throwable $e) {}
                $imagesSize += $size;
    ...
    private function getStorageStats(): array
    {
        ...
        foreach ($mediaFiles as $mf) {
            if ($mf->recipient_image) {
                try { $uploads += Storage::disk(config('filesystems.media_disk'))->size($mf->recipient_image); } catch (\Throwable $e) {}
            }
            if ($mf->background_music && !str_starts_with($mf->background_music, 'http')) {
                try { $uploads += Storage::disk(config('filesystems.media_disk'))->size($mf->background_music); } catch (\Throwable $e) {}
            }
        }
        $users = \App\Models\User::whereNotNull('profile_picture')->get();
        foreach ($users as $u) {
                ...
                foreach ($expired as $msg) {
                    ...
                    if ($msg->mediaFiles) {
                        if ($msg->mediaFiles->recipient_image) {
                            try { Storage::disk(config('filesystems.media_disk'))->delete($msg->mediaFiles->recipient_image); } catch (\Throwable $e) {}
                        }
                        if ($msg->mediaFiles->background_music) {
                            try { Storage::disk(config('filesystems.media_disk'))->delete($msg->mediaFiles->background_music); } catch (\Throwable $e) {}
                        }
